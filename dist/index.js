...
app.get("/items/:id", (req, res) => {
    connection.query("SELECT * FROM items WHERE id=?", [req.params.id], (err, data, fields) => {
        if (err)
            throw err;
        connection.query("SELECT * FROM comments WHERE item_id=" + String(data[0].id), (err, data2, field) => {
            if (err)
                throw err;
            data = data.map(function (a) {
                return Object.assign(Object.assign({}, a), { date_creating: stringData(a.date_creating) });
...
app.post("/login", (req, res) => {
    let redir = "/login";
    connection.query("SELECT * FROM users WHERE username=?", [[req.body.username]], (err, data, field) => {
        if (data[0] == undefined) {
...
app.post("/register", (req, res) => {
    ...
    if (req.body.username == "" || req.body.password == "") {
        ...
        connection.query("SELECT * FROM users WHERE username=?", [[req.body.username]], (err, data, fields) => {
            if (err)
                throw err;
            if (data[0] != undefined) {
                ...
                connection.query("INSERT INTO users (username, password, role) VALUES (?, ?, 'User')", [[req.body.username], md5(String([req.body.password]))], (err, data, field) => {
                    if (err)
                        throw err;
                    redir = "/";
                    req.session.auth = true;
...
app.post("/add", (req, res) => {
    ...
    connection.query("INSERT INTO items (title, image, description, price, author, date_creating) VALUES (?, ?, ?, ?, ?, ?)", [
        [req.body.title],
...
app.post("/update", (req, res) => {
    ...
    connection.query("UPDATE items SET title=?, image=?, description=?, price=? WHERE id=?", [
        [req.body.title],
        retImage(),
        [req.body.description],
        [Number(req.body.price)],
...
app.post("/delete", (req, res) => {
    ...
    connection.query("DELETE FROM items WHERE id=?", [Number([req.body.id])], (err, data, fields) => {
        if (err)
            throw err;
    });
    connection.query("DELETE FROM comments WHERE item_id=?", [Number([req.body.id])], (err, data, fields) => {
        if (err)
            throw err;
        res.redirect("/");
    });
...
app.post("/addCommentary", (req, res) => {
    if (req.body.commentary != "") {
        let date = new Date();
        connection.query("INSERT INTO comments(author, commentary, date_creating, item_id) VALUES (?, ?, ?, ?)", [req.session.username, [req.body.commentary], new Date(), String([req.body.id])], (err, data, fields) => {
            if (err)
                throw err;
        });
    }
...
app.post("/deleteCommentary", (req, res) => {
    connection.query("DELETE FROM comments WHERE id=?", [Number([req.body.idComment])], (err, data, fields) => {
        if (err)
            throw err;
    });
    res.redirect("/items/" + String([req.body.id]));
...
app.post("/usingSortAlfabet", (req, res) => {
    if (req.session.sortType == 1) {
        req.session.sortType = -1;
    }
    else {
        req.session.sortType = 1;
    }
...
app.post("/usingSortDate", (req, res) => {
    if (req.session.sortType == 2) {
        req.session.sortType = -2;
    }
    else {
        req.session.sortType = 2;
    }
...
app.post("/usingSortPrice", (req, res) => {
    if (req.session.sortType == 3) {
        req.session.sortType = -3;
    }
    else {
        req.session.sortType = 3;
    }
